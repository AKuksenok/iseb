...
def cal_pop_fitness(equation_inputs, pop):
    ...
    # The fitness function caulcuates the sum of products
    #  between each input and its corresponding weight.
    t = pop*equation_inputs
    fitness = numpy.sum(t, axis=1)
    return fitness
...
def select_mating_pool(pop, fitness, num_parents):
    ...
    for parent_num in range(num_parents):
        max_fitness_idx = numpy.asarray(fitness == numpy.max(fitness)).nonzero()
